...
def train_model(model: torch.nn.Module, train_loader: torch.utils.data.DataLoader, criterion: torch.nn.Module,
                optimizer: optim.Optimizer, scheduler: lr_scheduler._LRScheduler, checkpoint_path: str, 
                num_epochs: int, device: torch.device, patience=30) -> None:
    """
    ...
    for epoch in range(num_epochs):
        ...
        if avg_loss < best_train_loss:
            ...
            print(f"New best model saved at epoch {epoch+1} with Loss: {best_train_loss:.4f}")
        else:
            no_improve_count += 1
